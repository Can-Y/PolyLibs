...
{numbers_text}
    ...
    def generate_footprint(
        ...
        il_content = "\n".join(lines)
        il_filename = f"{pkg_name}.il"
        load_filename = f"{pkg_name}_load.txt"
        device_dir = device.full_name
        load_content = (
            f"; ============================================================\n"
            f"; SKILL Load Script — {pkg_name}\n"
            f"; Generated by PolyLibs\n"
            f"; ============================================================\n"
            f";\n"
            f"; 用法：\n"
            f"; 1. 打开 Allegro PCB Editor\n"
            f"; 2. File > New > Package Symbol，输入任意名称，进入编辑界面\n"
            f"; 3. 在下方 SKILL 命令行粘贴以下命令并回车：\n"
            f";\n"
            f'skill load("__POLYLIBS_IL_PATH__")\n'
            f";\n"
            f"; 脚本自动创建 padstack、全部焊盘、外形框与 A1 标记；\n"
            f"; File > Save 得 .dra。\n"
            f"; 若提示 padstack 已存在的 WARN，属正常（复用已有盘）。\n"
        )
        return {il_filename: il_content, load_filename: load_content}
